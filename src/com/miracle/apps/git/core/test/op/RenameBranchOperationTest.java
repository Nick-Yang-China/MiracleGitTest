...
import com.miracle.apps.git.core.op.RenameBranchOperation;
import com.miracle.apps.git.test.core.GitTestCase;
import com.miracle.apps.git.core.RepositoryUtil;

public class RenameBranchOperationTest extends GitTestCase {
	...
	@Test
	public void testRenameBranchOperation() throws Exception {
		...
		//rename branch of test1 to test2
	...
	@Test
	public void testRenameBranchOperationWithCheckOut() throws Exception {
		File file=new File(workdir,"file1.txt");
		FileUtils.createNewFile(file);
		...
		//create branch of "test1" and check out
		
		CreateLocalBranchOperation branchCre=new CreateLocalBranchOperation(repository, "test1", repository.getAllRefs().get(MASTER), null);
		branchCre.setCheckOutFlag(true).execute();
		
		assertEquals("test1", repository.getBranch());
